...
Put simply, a pix2pix networks extends the traditional GAN by some kind of input data. This is useful for the general purpose image-to-image translation. The discriminator is provided with both input and the target (or generated) image and must decide whether the target is a plausible transformation of the source image. While GAN attempts to learn a function mapping some random variable $z$ to output image $y$: $G: z → y$, a pix2pix searches a a function that maps an input image $x$, together with some random variable $z$ to output image $y$: $G: (x,z) → y$
